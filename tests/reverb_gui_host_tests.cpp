...
struct Output {
    ...
    void balanced() {
        std::array<int, reverb::parameterCount> depth{};
        for (unsigned n = 0; n < count; ++n) {
            auto &e = events[n];
            const int i = reverb::indexForId(e.param_id);
            CHECK(i >= 0);
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        Host host;
        const auto *p = factory->create_plugin(factory, &host.host, "org.openfilter.reverb");
        CHECK(p && p->init(p));
        ...
        for (unsigned cycle = 0; cycle < 5; ++cycle) {
            ...
            button(ButtonPress, 854, 88);
            button(ButtonRelease, 854, 88);
            ...
            CHECK(out.count == 0);
            double enabled = 0;
            CHECK(params->get_value(p, reverb::AutoGate, &enabled) &&
                  enabled ==
            ...
            const auto *other =
                factory->create_plugin(factory, &host.host, "org.openfilter.reverb");
            ...
            CHECK(otherParams->get_value(other, reverb::AutoGate, &enabled) &&
            ...
            automated.param_id = reverb::Predelay;
            automated.value = 55.;
            ...
            out.count = 0;
            time += 500;
            button(ButtonPress, 54, 164);
            button(ButtonRelease, 54, 164);
            time += 120;
            button(ButtonPress, 54, 164);
            button(ButtonRelease, 54, 164);
            realtime = true;
            ...
            realtime = false;
            double resetValue = 0;
            CHECK(params->get_value(p, reverb::Predelay, &resetValue));
            near(resetValue, 20.);
            out.balanced();
            tick();
            ...
            // events. Pointer jitter emits no gesture; retry preserves B/V/E.
            {
                ...
                out.capacity = 0;
                time += 500;
                constexpr unsigned readoutId = reverb::Output;
