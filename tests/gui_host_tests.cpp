...
struct Output {
    ...
    void balanced() {
        std::array<int, eq::parameterCount> depth{};
        for (unsigned n = 0; n < count; ++n) {
            auto &e = events[n];
            const int i = eq::indexForId(e.param_id);
            CHECK(i >= 0);
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        Host host;
        const auto *p = factory->create_plugin(factory, &host.host, "org.openfilter.eq");
        CHECK(p && p->init(p));
        ...
        for (unsigned cycle = 0; cycle < 5; ++cycle) {
            ...
            // Add Band uses a real X11 event -> Pugl -> editor -> CLAP event queue.
            button(ButtonPress, 60, 702);
            button(ButtonRelease, 60, 702);
            ...
            CHECK(out.count == 0);
            double enabled = 0;
            CHECK(params->get_value(p, eq::bandId(cycle, eq::Enabled), &enabled) &&
                  enabled == 1); // Queries reflect pending UI edits immediately.
            const auto *state =
            ...
            const auto *other = factory->create_plugin(factory, &host.host, "org.openfilter.eq");
            ...
            CHECK(otherParams->get_value(other, eq::bandId(cycle, eq::Enabled), &enabled) &&
                  enabled == 1);
            other->destroy(other);
            out.capacity = 1;
            realtime = true;
            ...
            params->flush(p, nullptr, &out.out);
            realtime = false;
            CHECK(params->get_value(p, eq::bandId(cycle, eq::Enabled), &enabled) && enabled == 1);
            out.balanced();
            tick();
            if (cycle == 0) {
                ...
                out.count = 0;
                time += 500;
                button(ButtonPress, 968, 25);
                button(ButtonRelease, 968, 25);
                pendingState.position = 0;
                CHECK(state->load(p, &pendingState.in));
                realtime = true;
                params->flush(p, nullptr, &out.out);
                realtime = false;
                CHECK(params->get_value(p, eq::bandId(0, eq::Enabled), &enabled) && enabled == 1);
                out.balanced();
                tick();
            }
            ...
            automated.param_id = eq::bandId(cycle, eq::Frequency);
            automated.value = std::log2(2500.);
            ...
            out.count = 0;
            time += 500;
            button(ButtonPress, 445, 552);
            button(ButtonRelease, 445, 552);
            time += 120;
            button(ButtonPress, 445, 552);
            button(ButtonRelease, 445, 552);
            realtime = true;
            ...
            realtime = false;
            double resetValue = 0;
            CHECK(params->get_value(p, eq::bandId(cycle, eq::Frequency), &resetValue));
            near(resetValue, std::log2(1000.));
            out.balanced();
            tick();
            ...
            // events. Pointer jitter emits no gesture; retry preserves B/V/E.
            {
                ...
                out.capacity = 0;
                time += 500;
                const unsigned readoutId = eq::parameter(1).id;
