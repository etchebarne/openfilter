...
struct Output {
    ...
    void balanced() {
        std::array<int, saturator::parameterCount> depth{};
        for (unsigned n = 0; n < count; ++n) {
            auto &e = events[n];
            const int i = saturator::indexForId(e.param_id);
            CHECK(i >= 0);
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        Host host;
        const auto *p = factory->create_plugin(factory, &host.host, "org.openfilter.saturator");
        CHECK(p && p->init(p));
        ...
        for (unsigned cycle = 0; cycle < 5; ++cycle) {
            ...
            // Bypass uses a real X11 event -> Pugl -> editor -> CLAP event queue.
            button(ButtonPress, 1060, 696);
            button(ButtonRelease, 1060, 696);
            ...
            CHECK(out.count == 0);
            double enabled = 0;
            CHECK(params->get_value(p, saturator::Bypass, &enabled) &&
                  enabled ==
            ...
            const auto *other =
                factory->create_plugin(factory, &host.host, "org.openfilter.saturator");
            ...
            CHECK(otherParams->get_value(other, saturator::Bypass, &enabled) &&
            ...
            automated.param_id = saturator::band(1, saturator::Drive);
            automated.value = 30.;
            ...
            out.count = 0;
            time += 500;
            button(ButtonPress, 435, 480);
            button(ButtonRelease, 435, 480);
            time += 120;
            button(ButtonPress, 435, 480);
            button(ButtonRelease, 435, 480);
            realtime = true;
            ...
            realtime = false;
            double resetValue = 0;
            CHECK(params->get_value(p, saturator::band(1, saturator::Drive), &resetValue));
            near(resetValue, 6.);
            out.balanced();
            tick();
