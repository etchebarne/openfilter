...
struct Fixture {
    ...
    explicit Fixture(const clap_plugin_factory *factory)
        : p(factory->create_plugin(factory, &host, "org.openfilter.deesser")) {
        CHECK(p && p->init(p));
...
void metadata(const clap_plugin_factory *factory) {
    ...
    f.start(44100);
    CHECK(latency->get(f.p) == 662);
    f.stop();
    f.start(96000);
    CHECK(latency->get(f.p) == 1440);
}
std::vector<double> automated(const clap_plugin_factory *factory, unsigned block) {
    ...
    timeline.add(1301, Range, 4);
    timeline.add(1451, Low, 11000);
    timeline.add(1509, High, 7000);
    timeline.add(1601, Low, -2000, true);
    timeline.add(1703, Bypass, 1);
    timeline.add(2201, Bypass, 0);
    timeline.add(2500, Threshold, 0, true);
    timeline.add(3007, Detection, 1);
    timeline.add(4001, Processing, 0);
...
void audio(const clap_plugin_factory *factory) {
    ...
    // Exact event offset: output trim starts a 480-sample ramp at frame 901.
    Fixture f(factory);
    Events setup;
    setup.add(0, Range, 0);
    f.flush(setup);
    f.start();
    Events change;
    change.add(901, Output, 6);
    std::vector<double> l(2048, .1), r = l;
    f.process(l, r, change);
    for (unsigned n = 720; n < 901; ++n)
        near(l[n], .1, 1e-15);
    CHECK(l[901] > .1);
    near(l[1381], .1 * std::pow(10., 6. / 20), 1e-12);
}
int main(int argc, char **argv) {
    try {
        ...
        std::cout << "Deesser CLAP: offsets, bit-identical partitions, mono/stereo float/double "
