...
struct Output {
    ...
    void balanced() {
        std::array<int, gate::parameterCount> depth{};
        for (unsigned n = 0; n < count; ++n) {
            auto &e = events[n];
            const int i = gate::indexForId(e.param_id);
            CHECK(i >= 0);
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        Host host;
        const auto *p = factory->create_plugin(factory, &host.host, "org.openfilter.gate");
        CHECK(p && p->init(p));
        ...
        for (unsigned cycle = 0; cycle < 5; ++cycle) {
            ...
            button(ButtonPress, 554, 630);
            button(ButtonRelease, 554, 630);
            ...
            CHECK(out.count == 0);
            double enabled = 0;
            CHECK(params->get_value(p, gate::Audition, &enabled) &&
                  enabled ==
            ...
            const auto *other = factory->create_plugin(factory, &host.host, "org.openfilter.gate");
            ...
            CHECK(otherParams->get_value(other, gate::Audition, &enabled) &&
            ...
            automated.param_id = gate::Attack;
            automated.value = 55.;
            ...
            out.count = 0;
            time += 500;
            button(ButtonPress, 886, 161);
            button(ButtonRelease, 886, 161);
            time += 120;
            button(ButtonPress, 886, 161);
            button(ButtonRelease, 886, 161);
            realtime = true;
            ...
            realtime = false;
            double resetValue = 0;
            CHECK(params->get_value(p, gate::Attack, &resetValue));
            near(resetValue, 1.);
            out.balanced();
            tick();
            ...
            // events. Pointer jitter emits no gesture; retry preserves B/V/E.
            {
                ...
                out.capacity = 0;
                time += 500;
                constexpr unsigned readoutId = gate::Output;
