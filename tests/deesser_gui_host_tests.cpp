...
struct Output {
    ...
    void balanced() {
        std::array<int, deesser::parameterCount> depth{};
        for (unsigned n = 0; n < count; ++n) {
            auto &e = events[n];
            const int i = deesser::indexForId(e.param_id);
            CHECK(i >= 0);
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        Host host;
        const auto *p = factory->create_plugin(factory, &host.host, "org.openfilter.deesser");
        CHECK(p && p->init(p));
        ...
        for (unsigned cycle = 0; cycle < 5; ++cycle) {
            ...
            // Audition uses a real X11 event -> Pugl -> editor -> CLAP event queue.
            button(ButtonPress, 604, 647);
            button(ButtonRelease, 604, 647);
            ...
            CHECK(out.count == 0);
            double enabled = 0;
            CHECK(params->get_value(p, deesser::Audition, &enabled) &&
                  enabled ==
            ...
            const auto *other =
                factory->create_plugin(factory, &host.host, "org.openfilter.deesser");
            ...
            CHECK(otherParams->get_value(other, deesser::Audition, &enabled) &&
            ...
            automated.param_id = deesser::Range;
            automated.value = 18.;
            ...
            out.count = 0;
            time += 500;
            button(ButtonPress, 322, 490);
            button(ButtonRelease, 322, 490);
            time += 120;
            button(ButtonPress, 322, 490);
            button(ButtonRelease, 322, 490);
            realtime = true;
            ...
            realtime = false;
            double resetValue = 0;
            CHECK(params->get_value(p, deesser::Range, &resetValue));
            near(resetValue, 6.);
            out.balanced();
            tick();
            ...
            // events. Pointer jitter emits no gesture; retry preserves B/V/E.
            {
                ...
                out.capacity = 0;
                time += 500;
                constexpr unsigned readoutId = deesser::Output;
