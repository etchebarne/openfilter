...
struct Fixture {
    ...
    explicit Fixture(const clap_plugin_factory *factory)
        : p(factory->create_plugin(factory, &host, "org.openfilter.gate")) {
        CHECK(p && p->init(p));
...
std::vector<double> automated(const clap_plugin_factory *factory, unsigned block) {
    ...
    timeline.add(4001, Hysteresis, 1);
    timeline.add(4109, Sidechain, 0);
    timeline.add(4301, SidechainLP, 800);
    timeline.add(4401, SidechainHP, 120);
    timeline.add(4501, Audition, 1);
    timeline.add(5003, Audition, 0);
    timeline.add(5107, WetPan, -50);
    timeline.add(5209, DryPan, 80);
    timeline.add(5303, DryGain, -12);
    timeline.add(5401, Hold, 80);
...
int main(int argc, char **argv) {
    try {
        ...
        std::cout << "Gate CLAP: offsets, bit-identical partitions, mono/stereo float/double "
