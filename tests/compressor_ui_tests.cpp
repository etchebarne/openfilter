    ...
    try {
        readoutTests();
        {
            ...
            Editor e([&] { return state; },
                     ...
                     tap);
            double time = 1;
            const auto drawer = e.viewBounds(1);
            e.press(drawer.x + 20, drawer.y + 12, 0, 0, time);
            e.release(0, 0);
            time += .6;
            ...
            near(state.values[Attack], 10);
            e.key('z', PUGL_MOD_CTRL | PUGL_MOD_SHIFT);
            near(state.values[Attack], 12.5);
            click(Threshold, 40, 1);
            e.input("nonsense");
            e.key(PUGL_KEY_ENTER, 0);
            near(state.values[Threshold], -18);
            e.key(PUGL_KEY_ESCAPE, 0);
            ...
            e.press(graph.x + graph.w * .18, graph.y + 10, 0, 0, time);
            e.motion(graph.x + graph.w * .09, graph.y + 20, 0);
            e.release(0, 0);
            time += .6;
            near(state.values[Threshold], -45);
            // Double-clicking the exposed knee follows the suite's default-reset rule.
            e.press(graph.x + graph.w * .18, graph.y + 12, 0, 0, time);
            e.release(0, 0);
            e.press(graph.x + graph.w * .18, graph.y + 12, 0, 0, time + .12);
            e.release(0, 0);
            time += .6;
            ...
            r = e.controlBounds(SidechainHP);
            e.press(r.x + 10, r.y + 29, 0, 0, time);
            e.motion(r.x + 30, r.y + 29, 0);
            e.release(0, 0);
            time += .6;
        ...
        FcFini();
        std::cout << "Compressor UI: all defaults, exact entry, undo/redo, fine drag, state reset "
