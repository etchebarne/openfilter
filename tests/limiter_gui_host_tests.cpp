...
struct Output {
    ...
    void balanced() {
        std::array<int, limiter::parameterCount> depth{};
        for (unsigned n = 0; n < count; ++n) {
            auto &e = events[n];
            const int i = limiter::indexForId(e.param_id);
            CHECK(i >= 0);
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        Host host;
        const auto *p = factory->create_plugin(factory, &host.host, "org.openfilter.limiter");
        CHECK(p && p->init(p));
        ...
        for (unsigned cycle = 0; cycle < 5; ++cycle) {
            ...
            button(ButtonPress, 200, 697);
            button(ButtonRelease, 200, 697);
            ...
            CHECK(out.count == 0);
            double enabled = 0;
            CHECK(params->get_value(p, limiter::AutoRelease, &enabled) &&
                  enabled ==
            ...
            const auto *other =
                factory->create_plugin(factory, &host.host, "org.openfilter.limiter");
            ...
            CHECK(otherParams->get_value(other, limiter::AutoRelease, &enabled) &&
            ...
            automated.param_id = limiter::Release;
            automated.value = 55.;
            ...
            out.count = 0;
            time += 500;
            button(ButtonPress, 453, 585);
            button(ButtonRelease, 453, 585);
            time += 120;
            button(ButtonPress, 453, 585);
            button(ButtonRelease, 453, 585);
            realtime = true;
            ...
            realtime = false;
            double resetValue = 0;
            CHECK(params->get_value(p, limiter::Release, &resetValue));
            near(resetValue, 200.);
            out.balanced();
            tick();
            ...
            // events. Pointer jitter emits no gesture; retry preserves B/V/E.
            {
                ...
                out.capacity = 0;
                time += 500;
                constexpr unsigned readoutId = limiter::Ceiling;
