...
const clap_plugin_descriptor descriptor{CLAP_VERSION,
                                        ...
                                        "",
                                        "0.3.0",
                                        "Predictive sample-peak limiter with stereo linking",
                                        features};
  ...
  private:
    ...
    template <class T> void render(const clap_process *p, T **in, T **out) noexcept {
        ...
        for (uint32_t frame = 0; frame < p->frames_count; ++frame) {
            ...
            const bool truePeak = base_[TruePeak] != 0;
            const double values[]{il, ir, truePeak ? engine_.outputPeak(0) : l,
                                  truePeak         ? engine_.outputPeak(1)
                                  : channels_ == 2 ? r
                                                   : l};
            for (unsigned c = 0; c < 4; ++c)
                peaks_[c] = std::max(std::isfinite(values[c]) ? std::abs(values[c]) : 0.,
                                     peaks_[c] * meterDecay_);
            clipped_ = clipped_ || std::abs(values[2]) > 1 || std::abs(values[3]) > 1;
            if (analyze)
                analysis_.sample(engine_.inputPeak(), std::max(std::abs(l), std::abs(r)),
    ...
    uint32_t tailGet() const noexcept override { return engine_.tail(); }
    ...
    bool remoteControlsPageGet(uint32_t i, clap_remote_controls_page *p) noexcept override {
        if (i > 1 || !p)
            return false;
        *p = {};
        std::snprintf(p->section_name, sizeof(p->section_name), "Limiter");
        std::snprintf(p->page_name, sizeof(p->page_name), "Limiting");
        const clap_id ids[]{Gain,        Ceiling,   Release, StereoLink,
                            AutoRelease, UnityGain, Bypass,  CLAP_INVALID_ID};
        std::copy(std::begin(ids), std::end(ids), p->param_ids);
        if (i == 1) {
            p->page_id = 1;
            std::snprintf(p->page_name, sizeof(p->page_name), "Advanced");
            const clap_id advanced[]{Style,      Lookahead,   Attack,   Release,
                                     StereoLink, ReleaseLink, TruePeak, AutoRelease};
            std::copy(std::begin(advanced), std::end(advanced), p->param_ids);
        }
        return true;
    }
    ...
    bool stateSave(const clap_ostream *stream) noexcept override {
        ...
        std::memcpy(data.data(), "OFLMSTAT", 8);
    ...
    bool stateLoad(const clap_istream *stream) noexcept override {
        std::array<uint8_t, stateSize> data{};
        if (!plugin::readAll(stream, data.data(), 16) || std::memcmp(data.data(), "OFLMSTAT", 8))
            return false;
        const auto version = plugin::get32(data.data() + 8);
        const auto count = plugin::get32(data.data() + 12);
        if (!((version == 1 && count == 7) || (version == 2 && count == parameterCount)))
            return false;
        const size_t length = 16 + count * 12 + 4;
        if (!plugin::readAll(stream, data.data() + 16, length - 16) ||
            plugin::get32(data.data() + length - 4) != plugin::checksum(data.data(), length - 4))
            return false;
        Snapshot s{};
        s.values = defaults();
        if (version == 1) {
            s.values[Style] = Legacy;
            s.values[TruePeak] = 0;
        }
...
extern "C" CLAP_EXPORT const clap_plugin_entry clap_entry{
    CLAP_VERSION, openfilter::limiter::entryInit, openfilter::limiter::entryDeinit,
    openfilter::limiter::getFactory};
