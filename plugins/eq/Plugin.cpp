...
const char *const features[]{CLAP_PLUGIN_FEATURE_AUDIO_EFFECT, CLAP_PLUGIN_FEATURE_EQUALIZER,
  ...
  private:
    ...
    template <class T> void render(const clap_process *p, T **in, T **out) noexcept {
        ...
        for (uint32_t frame = 0; frame < p->frames_count; ++frame) {
            ...
            if (analyze)
                analysis_.sample(il, ir, l, channels_ == 2 ? r : l);
        }
    }
    ...
    bool audioPortsGetConfig(uint32_t i, clap_audio_ports_config *c) const noexcept override {
        ...
        *c = {};
        ...
        c->input_port_count = c->output_port_count = 1;
    ...
    uint32_t tailGet() const noexcept override { return std::numeric_limits<uint32_t>::max(); }
    ...
    bool paramsInfo(uint32_t i, clap_param_info *info) const noexcept override {
        ...
        *info = {};
        ...
        if (i >= 2 &&
            ((i - 2) % fields == Type || (i - 2) % fields == Slope || (i - 2) % fields == Routing))
        ...
        if (i >= 2)
            std::snprintf(info->module, sizeof(info->module), "Band %02u", (i - 2) / fields + 1);
        return true;
    }
    ...
    bool paramsValueToText(clap_id id, double value, char *text, uint32_t size) noexcept override {
        ...
        if (f == Type)
            label = shapeNames[static_cast<unsigned>(value)];
        if (f == Slope)
            label = slopeNames[static_cast<unsigned>(value)];
        if (f == Routing)
            label = routeNames[static_cast<unsigned>(value)];
        if (label) {
            std::snprintf(text, size, "%s", label);
            return true;
        }
        if (f == Frequency || f == Q)
            value = std::exp2(value);
        const char *suffix = f == Frequency ? " Hz" : i == 1 || f == Gain ? " dB" : "";
        char number[64]{};
        const auto result =
            std::to_chars(number, number + 63, value, std::chars_format::fixed, f == Q ? 4 : 2);
        if (result.ec != std::errc{})
            return false;
        *result.ptr = 0;
        std::snprintf(text, size, "%s%s", number, suffix);
        return true;
    }
    bool paramsTextToValue(clap_id id, const char *text, double *value) noexcept override {
        ...
        if (f == Type && match(shapeNames))
            return true;
        if (f == Routing && match(routeNames))
            return true;
        if (f == Slope && match(slopeNames))
            return true;
        if (i == 0 || f == Enabled) {
            if (input == "On") {
                *value = 1;
        ...
        if (f == Frequency) {
            if (suffix == "kHz" || suffix == "khz")
                parsed *= 1000;
            else if (!suffix.empty() && suffix != "Hz" && suffix != "hz")
                return false;
        } else if (i == 1 || f == Gain) {
            if (!suffix.empty() && suffix != "dB" && suffix != "db")
                return false;
        ...
        if (f == Frequency || f == Q) {
            if (parsed <= 0)
                return false;
            parsed = std::log2(parsed);
        }
        *value = parameter(i).constrain(parsed);
        return true;
    }
    ...
    uint32_t remoteControlsPageCount() noexcept override { return bands + 1; }
    bool remoteControlsPageGet(uint32_t i, clap_remote_controls_page *p) noexcept override {
        if (i > bands || !p)
            return false;
        *p = {};
        p->page_id = i;
        for (auto &id : p->param_ids)
            id = CLAP_INVALID_ID;
        std::snprintf(p->section_name, sizeof(p->section_name), "%s", i ? "Bands" : "Global");
        if (!i) {
            std::snprintf(p->page_name, sizeof(p->page_name), "Output");
            p->param_ids[0] = 1;
            p->param_ids[1] = 0;
        } else {
            std::snprintf(p->page_name, sizeof(p->page_name), "Band %02u", i);
            constexpr Field order[]{Frequency, Gain, Q, Type, Enabled, Slope, Routing};
            for (unsigned k = 0; k < fields; ++k)
                p->param_ids[k] = bandId(i - 1, order[k]);
            p->param_ids[7] = 1;
        }
        return true;
    }
    ...
    bool stateSave(const clap_ostream *stream) noexcept override {
        ...
        std::memcpy(data.data(), "OFEQSTAT", 8);
    ...
    bool stateLoad(const clap_istream *stream) noexcept override {
        ...
        if (!plugin::readAll(stream, data.data(), data.size()) ||
            std::memcmp(data.data(), "OFEQSTAT", 8) || plugin::get32(data.data() + 8) != 1 ||
...
extern "C" CLAP_EXPORT const clap_plugin_entry clap_entry{CLAP_VERSION, openfilter::eq::entryInit,
                                                          openfilter::eq::entryDeinit,
                                                          openfilter::eq::getFactory};
