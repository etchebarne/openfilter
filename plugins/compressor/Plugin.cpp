...
const char *const features[]{CLAP_PLUGIN_FEATURE_AUDIO_EFFECT, CLAP_PLUGIN_FEATURE_COMPRESSOR,
                             CLAP_PLUGIN_FEATURE_STEREO, CLAP_PLUGIN_FEATURE_MONO, nullptr};
const clap_plugin_descriptor descriptor{CLAP_VERSION,
                                        "org.openfilter.compressor",
                                        "OpenFilter Compressor",
                                        "OpenFilter",
                                        "",
                                        "",
                                        "",
                                        "0.1.2",
                                        "Feed-forward dynamics with lookahead and sidechain",
                                        features};
  ...
  private:
    ...
    template <class T> void render(const clap_process *p, T **in, T **out) noexcept {
        ...
        for (uint32_t frame = 0; frame < p->frames_count; ++frame) {
            ...
            if (analyze)
                analysis_.sample(std::max(std::abs(il), std::abs(ir)),
                                 std::max(std::abs(l), std::abs(r)), engine_.gainReduction());
        }
    }
    ...
    uint32_t tailGet() const noexcept override { return engine_.latency(); }
    ...
    bool remoteControlsPageGet(uint32_t i, clap_remote_controls_page *p) noexcept override {
        ...
        *p = {};
        p->page_id = i;
        const char *names[]{"Compression", "Timing / sidechain", "Gain / mix"};
        std::snprintf(p->section_name, sizeof(p->section_name), "Compressor");
        std::snprintf(p->page_name, sizeof(p->page_name), "%s", names[i]);
        const unsigned ids[3][8]{
            {Threshold, Ratio, Attack, Release, Knee, Range, Makeup, Mix},
            {Detector, AutoRelease, Hold, Lookahead, Sidechain, SidechainHP, StereoLink, Bypass},
            {Input, Output, Makeup, Mix, Threshold, Ratio, StereoLink, Bypass}};
    ...
    bool stateSave(const clap_ostream *stream) noexcept override {
        ...
        std::memcpy(data.data(), "OFCPSTAT", 8);
    ...
    bool stateLoad(const clap_istream *stream) noexcept override {
        ...
        if (!plugin::readAll(stream, data.data(), data.size()) ||
            std::memcmp(data.data(), "OFCPSTAT", 8) || plugin::get32(data.data() + 8) != 1 ||
...
extern "C" CLAP_EXPORT const clap_plugin_entry clap_entry{
    CLAP_VERSION, openfilter::compressor::entryInit, openfilter::compressor::entryDeinit,
    openfilter::compressor::getFactory};
