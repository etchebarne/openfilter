...
const char *const features[]{CLAP_PLUGIN_FEATURE_AUDIO_EFFECT, CLAP_PLUGIN_FEATURE_DISTORTION,
                             CLAP_PLUGIN_FEATURE_STEREO, CLAP_PLUGIN_FEATURE_MONO, nullptr};
const clap_plugin_descriptor descriptor{CLAP_VERSION,
                                        "org.openfilter.saturator",
                                        "OpenFilter Saturator",
                                        "OpenFilter",
                                        "",
                                        "",
                                        "",
...
class Plugin final : public Base {
  ...
  private:
    ...
    uint32_t tailGet() const noexcept override {
        // Crossover, tone and residual DC filters can ring beyond the FIR delay.
    ...
    uint32_t remoteControlsPageCount() noexcept override { return 4; }
    bool remoteControlsPageGet(uint32_t i, clap_remote_controls_page *p) noexcept override {
        if (i >= 4 || !p)
            return false;
        *p = {};
        p->page_id = i;
        std::snprintf(p->section_name, sizeof(p->section_name), "Saturator");
        std::snprintf(p->page_name, sizeof(p->page_name), i ? "Band %u" : "Global", i);
        ...
        if (!i) {
            ...
            for (unsigned k = 0; k < 8; ++k)
                p->param_ids[k] = band(i - 1, fields[k]);
        }
        return true;
    }
...
extern "C" CLAP_EXPORT const clap_plugin_entry clap_entry{
    CLAP_VERSION, openfilter::saturator::entryInit, openfilter::saturator::entryDeinit,
    openfilter::saturator::getFactory};
