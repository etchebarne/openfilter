...
const clap_plugin_descriptor descriptor{
    ...
    "",
    "0.1.0",
    "Frequency-selective de-essing with vocal detection and audition",
    features};
  ...
  private:
    ...
    template <class T> void render(const clap_process *p, T **in, T **out) noexcept {
        ...
        for (uint32_t frame = 0; frame < p->frames_count; ++frame) {
            ...
            if (analyze)
                analysis_.sample(engine_.displayInput(0), engine_.displayInput(1),
                                 engine_.gainReduction(), engine_.displayDetector(0),
                                 engine_.displayDetector(1));
        }
    }
    ...
    bool remoteControlsPageGet(uint32_t i, clap_remote_controls_page *p) noexcept override {
        ...
        *p = {};
        p->page_id = i;
        const char *names[]{"De-essing", "Detection / timing", "Gain / monitoring"};
        std::snprintf(p->section_name, sizeof(p->section_name), "Deesser");
        std::snprintf(p->page_name, sizeof(p->page_name), "%s", names[i]);
        const unsigned ids[3][8]{
            {Threshold, Range, Low, High, Detection, Processing, Audition, Bypass},
            {Attack, Release, Lookahead, StereoLink, Sidechain, Low, High, Audition},
            {Input, Output, Threshold, Range, StereoLink, Processing, Audition, Bypass}};
    ...
    bool stateSave(const clap_ostream *stream) noexcept override {
        ...
        std::memcpy(data.data(), "OFDSSTAT", 8);
    ...
    bool stateLoad(const clap_istream *stream) noexcept override {
        ...
        if (!plugin::readAll(stream, data.data(), data.size()) ||
            std::memcmp(data.data(), "OFDSSTAT", 8) || plugin::get32(data.data() + 8) != 1 ||
...
extern "C" CLAP_EXPORT const clap_plugin_entry clap_entry{
    CLAP_VERSION, openfilter::deesser::entryInit, openfilter::deesser::entryDeinit,
    openfilter::deesser::getFactory};
