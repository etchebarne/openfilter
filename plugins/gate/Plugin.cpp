...
const char *const features[]{CLAP_PLUGIN_FEATURE_AUDIO_EFFECT, CLAP_PLUGIN_FEATURE_GATE,
                             CLAP_PLUGIN_FEATURE_EXPANDER,     CLAP_PLUGIN_FEATURE_STEREO,
                             CLAP_PLUGIN_FEATURE_MONO,         nullptr};
const clap_plugin_descriptor descriptor{
    ...
    "",
    "0.1.0",
    "Downward gate / expander with lookahead and filtered sidechain",
    features};
...
class Plugin final : public Base {
  ...
  private:
    ...
    template <class T> void render(const clap_process *p, T **in, T **out) noexcept {
        ...
        for (uint32_t frame = 0; frame < p->frames_count; ++frame) {
            ...
            if (p->audio_inputs_count > 1) {
                const auto &sc = p->audio_inputs[1];
                if constexpr (std::is_same_v<T, float>) {
                    sl = sc.data32[0][frame];
                    sr = sc.data32[sc.channel_count > 1 ? 1 : 0][frame];
                } else {
                    sl = sc.data64[0][frame];
                    sr = sc.data64[sc.channel_count > 1 ? 1 : 0][frame];
                }
            }
            engine_.sample(l, r, channels_ == 1, sl, sr);
            ...
            if (analyze)
                analysis_.sample(engine_.delayedInputPeak(), std::max(std::abs(l), std::abs(r)),
                                 engine_.gainReduction());
        }
    }
    clap_process_status process(const clap_process *p) noexcept override {
        if (!p || p->frames_count > maxFrames_ ||
            (p->audio_inputs_count < 1 || p->audio_inputs_count > 2) ||
        ...
        if (in.channel_count != channels_ || out.channel_count != channels_)
            return CLAP_PROCESS_ERROR;
        if (p->audio_inputs_count == 2) {
            const auto &sc = p->audio_inputs[1];
            if (sc.channel_count != channels_)
                return CLAP_PROCESS_ERROR;
    ...
    uint32_t audioPortsCount(bool input) const noexcept override { return input ? 2 : 1; }
    bool audioPortsInfo(uint32_t index, bool input,
                        clap_audio_port_info *info) const noexcept override {
        if (index >= (input ? 2u : 1u) || !info)
            return false;
        *info = {};
        info->id = input ? (index == 0 ? 0 : 2) : 1;
        std::snprintf(info->name, sizeof(info->name), "%s",
                      input ? (index == 0 ? "Input" : "Sidechain") : "Output");
    ...
    uint32_t tailGet() const noexcept override {
        // Filtered sidechain audition can ring beyond the program delay.
        // Keep the host processing rather than truncate that IIR tail.
    ...
    bool remoteControlsPageGet(uint32_t i, clap_remote_controls_page *p) noexcept override {
        ...
        *p = {};
        p->page_id = i;
        const char *names[]{"Gate / expander", "Timing / sidechain", "Gain / mix"};
        std::snprintf(p->section_name, sizeof(p->section_name), "Gate");
        std::snprintf(p->page_name, sizeof(p->page_name), "%s", names[i]);
        const unsigned ids[3][8]{
            {Threshold, Ratio, Attack, Release, Knee, Range, WetGain, Mix},
            {Detector, Hysteresis, Hold, Lookahead, Sidechain, SidechainHP, StereoLink, Bypass},
            {Input, Output, WetGain, DryGain, WetPan, DryPan, SidechainLP, Audition}};
    ...
    bool stateSave(const clap_ostream *stream) noexcept override {
        ...
        std::memcpy(data.data(), "OFGTSTAT", 8);
    ...
    bool stateLoad(const clap_istream *stream) noexcept override {
        ...
        if (!plugin::readAll(stream, data.data(), data.size()) ||
            std::memcmp(data.data(), "OFGTSTAT", 8) || plugin::get32(data.data() + 8) != 1 ||
...
extern "C" CLAP_EXPORT const clap_plugin_entry clap_entry{CLAP_VERSION, openfilter::gate::entryInit,
                                                          openfilter::gate::entryDeinit,
                                                          openfilter::gate::getFactory};
