...
enum class Brand { Eq, Compressor, Limiter, Reverb, Deesser, Gate, Saturator };
...
inline void drawBrand(cairo_t *c, Brand brand, Rect destination) {
    ...
    const auto bounds = brand == Brand::Eq           ? brand_detail::eqBounds
                        ...
                        : brand == Brand::Saturator  ? brand_detail::saturatorBounds
    ...
    else if (brand == Brand::Limiter)
        brand_detail::limiter(c);
    else if (brand == Brand::Reverb)
        brand_detail::reverb(c);
    else if (brand == Brand::Saturator)
        brand_detail::saturator(c);
